...
self.onmessage = async (e: MessageEvent) => {
  switch (e.data.type) {
    case "init": {
      ...
      trace(
        `init done, radioKey="${e.data.radioKey}", hasContent=${backend.hasContent}`,
      );
    ...
    case "deleteFile": {
      try {
        await backend.unlink(e.data.path);
        self.postMessage({ type: "deleteFileDone", id: e.data.id });
      } catch (err: any) {
        self.postMessage({
          type: "deleteFileDone",
          id: e.data.id,
    ...
    case "deleteDir": {
      try {
        // Recursively delete all files first then the dir
        const files = backend.listFiles(e.data.path);
        for (const f of files) {
          await backend.unlink(f);
        }
        // Remove empty subdirs bottom-up
        const allDirs: string[] = [];
        function collectDirs(p: string) {
          try {
            const entries = backend.readdir(p, true) as any[];
            for (const ent of entries) {
              if (ent.isDirectory()) {
                const childPath =
                  p === "/" ? `/${ent.name}` : `${p}/${ent.name}`;
                collectDirs(childPath);
                allDirs.push(childPath);
              }
            }
          } catch {}
        }
        collectDirs(e.data.path);
        ...
        try {
          await backend.rmdir(e.data.path);
        } catch {}
        self.postMessage({ type: "deleteDirDone", id: e.data.id });
      } catch (err: any) {
        self.postMessage({
          type: "deleteDirDone",
          id: e.data.id,
...
function dispatch(ch: Channel): (() => Promise<void>) | null {
  ...
  try {
    switch (opcode) {
      ...
      case FsOp.LINK:
        throw Object.assign(new Error("ENOSYS: hardlinks not supported"), {
          code: "ENOSYS",
        });

      case FsOp.SYMLINK:
        throw Object.assign(new Error("ENOSYS: symlinks not supported"), {
          code: "ENOSYS",
        });
      ...
      default:
        throw Object.assign(new Error(`unsupported fs op: ${opcode}`), {
          code: "ENOSYS",
        });
    }
  } catch (e: any) {
