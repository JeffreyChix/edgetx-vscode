...
import FsWorkerUrl from './fs-worker.ts?worker&inline';
import WasmWorkerClass from './worker.ts?worker&inline';
...
export class WasmRunner {
  ...
  async initFs(radioKey: string): Promise<{ hasContent: boolean }> {
    this.fsWorker = new (FsWorkerUrl as any)();
  ...
  async load(wasmPath: string): Promise<void> {
    if (!this.fsWorker)
      throw new Error("initFs() must be called before load()");
    ...
    this.wasiThreads = new WASIThreads({
      ...
      onCreateWorker: () => {
        const worker = new (WasmWorkerClass as any)();
        worker.postMessage({
          type: "analog-buffer",
    ...
    const { module, instance } = await WebAssembly.instantiateStreaming(clone, {
  ...
  async fsWriteFile(path: string, data: ArrayBuffer): Promise<void> {
    // Copy to avoid transferring the original buffer
    const copy = data.slice(0);
    await this.fsMessage("writeFile", { path, data: copy }, [copy]);
  }
  ...
  async fsDeleteFile(path: string): Promise<void> {
    await this.fsMessage("deleteFile", { path });
  }

  async fsDeleteDir(path: string): Promise<void> {
    await this.fsMessage("deleteDir", { path });
  }
