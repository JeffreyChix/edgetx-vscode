...
local raw_yaw = nil
local fm_id = -1
local drone_heading = 0
local pilot_heading = 0

local function readPilotHeading()
    local data = serialRead() or ""

    local val = string.match(data, "H:([%d%.]+)")
    if val then
        pilot_heading = tonumber(val)
    end
end
